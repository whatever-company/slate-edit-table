 ...
 * It unwrap blocks in cell blocks
 */
function alwaysBlocksWithinCell(opts: Options): Rule {
    return {
        match(node) {
            return node.object == 'block' && node.type == opts.typeCell;
        },
        ...
        // Find nested blocks
        validate(node) {
            const toWrap = node.nodes.filter(child => child.object !== 'block');
        ...
        normalize(transform, node, toWrap) {
            transform = toWrap.reduce(
                (tr, unwrapped) =>
                    tr.wrapBlockByKey(unwrapped.key, opts.typeContent, {
                        normalize: false
                    }),
                transform
            );
